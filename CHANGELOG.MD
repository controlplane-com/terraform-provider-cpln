...
- Add canary block to domain route resource and data source.
- Add alias_workload_link to GVC resource and data source.
- Add loki logging provider to org logging.
- Add static validator bounds to org resource.
- Add ingress_replicas to mk8s BYOK middlebox configuration.
- Add cpu_options to mk8s AWS node pool.
- Add k8s:// scheme to workload volume uri.
- Add object defaults to workload VM firmware, network, and clock blocks.
- Add static validators to workload VM checksum, volume set, ssh key, and user fields.
- Default workload liveness probe initial delay to 60 seconds.
- Align workload autoscaling and firewall bounds with the API.
- Make volume set performance_class optional.
- Fix parent_id JSON tag on volume set status.
- Make identity native network resource fqdn optional.
- Add static validators to identity network resources and access policies.
- Make secret tls key optional.
- Add format validators to nats-account secret.
- Fix external_id output on ecr secret data source.
- Add max length validator to service account key description.
- Add reserved port 15001 to port validators.

## 1.2.30

- Add vm type and vm block to workload resource and data source.
- Add health block to workload data source.
- Add kubevirt addon to mk8s BYOK addon configuration.
- Add node_local_dns addon to mk8s BYOK addon configuration.

## 1.2.29

- Extend workload termination grace period validation.

## 1.2.28

- Add location_query block to GVC resource and data source.
- Fix inconsistent plan results by preserving user-chosen self-link forms across resources.

## 1.2.27

- Bump dependencies to patch jwt/v5 and quic-go vulnerabilities.
- Add location_options to GVC data source.
- Fix failures and successes JSON tags on workload health check status.
- Add suspended_status to workload status.
- Add run_as_user to workload security_options.
- Add remote_write to mk8s BYOK monitoring configuration.
- Add external_labels to mk8s BYOK monitoring configuration.
- Add byok block with no_default_storage_classes to mk8s BYOK addon configuration.
- Add is_default and number_of_replicas to mk8s BYOK longhorn configuration.
- Add port and ip to mk8s BYOK middlebox configuration.
- Add origin, geo, and ip_ranges to custom location resource.
- Document origin and cloud_provider enums on location resource.
- Add location_options block to GVC resource.
- Correct origin description and validate language on group resource.
- Add port to domain route mirror block.
- Add status block to cloud account resource.
- Document origin enum on audit context resource.
- Add status protocol_version to agent resource.
- Add juicefs to mk8s BYOK addon configuration.
- Make domain port tls block optional.

## 1.2.26

- Add OpenTelemetry logging to org logging resource.
- Fix domain route import to accept domain name alongside full link.

## 1.2.25

- Remove schedules property from workload resource.

## 1.2.24

- Add predictive scaling to volume set autoscaling.
- Add mirror property to domain route.
- Add validation for KEDA fallback trigger requirements.
- Add schedules property to workload resource.

## 1.2.23

- Add per-domain mutex, inline routes, 409 retry, and remove domain sleep.

## 1.2.22

- Add headlamp addon to mk8s resource.

## 1.2.21

- Fix azure provider validator path expression error on image block in mk8s resource.
- Fix broken JSON tags for subscriptionId and zones in azure provider client structs.

## 1.2.20

- Add Allow as an option to workload job concurrency policy.
- Add fallback to workload keda.

## 1.2.19

- Sync GCP MK8s resource with latest API changes.

## 1.2.18

- Add `cpln_catalog_template` resource.

## 1.2.17

- Fix `objectName` value being shown as a reference in identity resource.

## 1.2.16

- Fix unnecessary plan diffs for domain resource.
- Fix node pool block replacement on slight changes in mk8s resource.
