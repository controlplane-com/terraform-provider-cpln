...
func (p *CplnProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	if config.Endpoint.IsNull() || config.Endpoint.IsUnknown() {
		if endpoint := os.Getenv("CPLN_ENDPOINT"); endpoint != "" {
			...
			config.Endpoint = types.StringValue("https://api.cpln.io")
		}
	}
	...
	// Create a new cpln c using the configuration values
	c, err := client.NewClient(config.Org.ValueString(), config.Endpoint.ValueString(), config.Profile.ValueString(), config.Token.ValueString(), config.RefreshToken.ValueString())
	...
	// Make the cpln c available during DataSource and Resource type Configure methods
