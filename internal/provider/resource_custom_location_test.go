...
func TestAccControlPlaneCustomLocation_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccControlPlaneCustomLocation("byok-loc-2", "desc-1", "byok", "true", "bar", "qux"),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("cpln_custom_location.new", "name", "byok-loc-2"),
			...
			{
				Config: testAccControlPlaneCustomLocation("byok-loc-2", "desc-2", "byok", "false", "bar-2", "qux-2"),
				Check: resource.ComposeTestCheckFunc(
...
func testAccCheckControlPlaneCustomLocationCheckDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		customLocation, _, _ := c.GetLocation(customLocationName)

		if customLocation != nil {
			return fmt.Errorf("Custom Location still exists. Name: %s", *customLocation.Name)
		}
	}
